        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {[7, 14, 30].map(d => (
                <button key={d} onClick={() => { setDays(d); setGenerating(false); setGenerateMsg(null) }}
            ...
            {generating && <span style={{ fontSize: 12, color: 'var(--text-muted)', display: 'inline-flex', alignItems: 'center', gap: 6 }}><Spinner size={13} muted inline />{coldStartSecs > 0 ? `Waking… ${coldStartSecs}s` : 'Building…'}</span>}
          </div>
        </div>

        {generateMsg && (() => {
          const s = generateMsg.type === 'success' ? 'good' : generateMsg.type === 'info' ? 'info' : 'alert'
          return <div style={{ padding: '10px 14px', borderRadius: 'var(--radius-sm)', fontSize: 13, background: `var(--status-${s}-bg)`, color: `var(--status-${s}-text)`, border: '1px solid var(--border)' }}>{generateMsg.text}</div>
        })()}
                  ...
                  <div style={{ flex: 1, overflowY: 'auto', padding: 10, display: 'flex', flexDirection: 'column', gap: 9 }}>
                    {cards.map(g => (
                      <div key={g.representative_id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 11, padding: '12px 13px', boxShadow: 'var(--shadow-card)' }}>
                        <div style={{ fontFamily: 'var(--font-ui)', fontSize: 12.5, fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{g.target_address}</div>
                        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--text-muted)', marginTop: 2 }}>{g.source_suburb || ''}</div>
                        {g.target_owner_name && <div style={{ fontFamily: 'var(--font-ui)', fontSize: 11, color: 'var(--text-muted)', marginTop: 8, paddingTop: 8, borderTop: '1px solid var(--border)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{g.target_owner_name}</div>}
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <Select
                        ...
                        options={[
                          ...
                          { value: 'listing_signed', label: 'Won — listed' },
    ...
    <form
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '10px', marginBottom: '10px' }}>
        <input
          ...
          style={{ padding: '8px 10px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontSize: '13px' }}
        />
        <Select
          required
          value={sourceSuburb}
