...
export default function Pipeline({ autoGeneratedFor, setAutoGeneratedFor }) {
  ...
  useEffect(() => {
    ...
    fetch(`${PIPELINE_API}/recent-sales?suburb=${encodeURIComponent(suburb)}&days=${days}`,
      ...
      .catch((err) => {
        // Aborted = a newer suburb/days fetch took over; keep loading
        // true so the new request owns the spinner.
    ...
    <div style={{ padding: '24px', maxWidth: '1280px', margin: '0 auto' }}>
      ...
      {loadError && !loading && (
        <div style={{
          ...
          <button
            type="button"
            onClick={() => loadTracking({ force: true })}
            style={{
      ...
      {recentSalesLoading && recentSales.length === 0 ? (
        <div style={{
          ...
          <span aria-hidden="true" style={{
            width: '14px', height: '14px', borderRadius: '50%',
            border: '2px solid rgba(107,114,128,0.25)', borderTopColor: '#6b7280',
            animation: 'sd-spin 0.8s linear infinite', flexShrink: 0,
          }} />
          <span>Loading recent sales for {suburb}… (first load can take 15–30s while the server wakes up)</span>
          <style>{`@keyframes sd-spin { to { transform: rotate(360deg) } }`}</style>
        </div>
