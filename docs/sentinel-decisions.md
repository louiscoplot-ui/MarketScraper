...
### D6 — Signal engine (S2) : lectures conservatrices des features
- `street_momentum` compte les VENTES (events `sold`) dans la rue — la détection
  de « ventes record » exigerait un benchmark de prix que la v1 n'a pas.
- `competitor_price_drops` ne score que les adresses actuellement en vente
  (c'est une campagne concurrente qui patine) ; `withdrawn_recent`,
  `long_hold_gain` et `street_momentum` ne scorent que le off-market.
- Refresh : une ligne vivante par (suburb, adresse normalisée) ; `new` mis à
  jour en place, `dismissed` protégé 30 j, `actioned` 90 j — le tri de l'agent
  n'est jamais écrasé par le cron.
- Colonne `normalized_address` ajoutée à `vendor_signals` (et `predictions`) —
  hors schéma du handoff mais nécessaire au matching S3 sans re-normaliser à
  chaque passe.

### D7 — Morning brief (S4)
- Opt-in : réutilise `users.digest_enabled` (même consentement que le digest) —
  pas de nouveau flag tant que le pilote n'en montre pas le besoin.
- Modèle narratif : `claude-sonnet-4-6` épinglé par le handoff (ID actif,
  vérifié), surchargeable via env `BRIEF_MODEL` sans redeploy.
- Appel API Claude en HTTP brut via `requests` (déjà en dépendance) — la règle
  « no new deps » exclut le SDK anthropic. Sans `ANTHROPIC_API_KEY` ou sur
  toute erreur/refus : fallback = reason_codes verbatim. Le brief part
  toujours, n'invente jamais, ne fait jamais échouer le cron.
- Pixel d'ouverture : route auth-exempt `/api/brief/open/<token>.gif`, token
  aléatoire 24 octets par brief (jamais l'id séquentiel), GET-only, ne révèle
  rien — exemption ajoutée à `_AUTH_EXEMPT_PREFIXES` (app.py).
- Lettres : builder dédié par type de signal (withdrawn ≠ street momentum ≠
  price drops ≠ long hold), profil agent réel uniquement (règle UX-1 : jamais
  de placeholder), téléchargement fetch+blob.
